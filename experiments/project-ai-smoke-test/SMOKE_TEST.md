...
- 架构评审任务应选择 `archsight-architecture-review` Skill，由 Atlas 作为主 Agent 承担职责归属。
...
| 能选择正确 Skill | 通过 | `.ai/skills.md` 已启用 `archsight-architecture-review`，并说明来源路径。 |
| 能输出结论 / 风险 / 后续动作 | 通过 | 本文件包含结论、风险与边界、后续动作。 |
| 不会要求 Hermes 才能工作 | 通过 | `.ai/project-context.md` 将 Runtime 明确为本地项目工作区，且注明不要求 Hermes。 |
| 不会把 Agent 当 Skill | 通过 | `.ai/skills.md` 和本文件均区分 Atlas 与 `archsight-architecture-review`。 |
