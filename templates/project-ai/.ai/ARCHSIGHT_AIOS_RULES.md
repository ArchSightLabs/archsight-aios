...
- 建筑行业能力通过 `.ai/profile-detection.md`、profile 和任务上下文启用；未启用时，不得把 BIM、IFC、GraphRAG 或审图场景当作项目默认事实，也不得把规范或工程证据链当作项目默认事实。
- 如果是否属于建筑行业任务不明确，先读取 `.ai/profile-detection.md`、`.ai/project-context.md`、README 和用户任务事实；仍不明确时按通用工程任务处理，并说明未启用行业增强。
