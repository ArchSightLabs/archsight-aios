# v1.4.0 Release Readiness

本清单用于 v1.4.0 正式发布前收口。当前 `package.json` 仍是 `1.3.2`，不得仅因代码合并就发布 `1.4.0`。

## 发布硬门槛

- [ ] `aios-tender-write` 和 `aios-scheme-write` 的写作样板已通过 `writing:validate`。
- [ ] `engineering-document-writing-scorecard.json` 已通过校验。
- [ ] Codex 真实触发 `aios-tender-write` 原始输出已归档。
- [ ] Codex 真实触发 `aios-scheme-write` 原始输出已归档。
- [ ] WorkBuddy 真实触发 `aios-tender-write` 原始输出已归档。
- [ ] WorkBuddy 真实触发 `aios-scheme-write` 原始输出已归档。
- [ ] 所有真实输出已按写作型 scorecard 复核，不把 prompt paste 结果冒充为 skill-runtime。
- [ ] 外部使用说明已更新，并优先展示 WorkBuddy-first 最短路径。

## 发布级验证命令

```bash
npm run validate:skills
npm run validate:prompts
npm run validate:prompt-run-pack
npm run validate:public-advisory-run-pack
npm run validate:document-writing-run-pack
npm run validate:prompt-run-results
npm run validate:prompt-outputs
npm run validate:prompt-scorecard
npm run validate:skill-runtime-evidence
npm run doctor
npm run smoke:project
npm test
npx skills add . --list
npm pack --dry-run
claude plugin validate .
git diff --check
```

如果某个宿主 CLI 不支持对应命令，应记录为宿主工具能力限制，不得写成仓库验证失败或通过。

## 版本同步点

正式 bump 到 `1.4.0` 时必须同步：

- `package.json`
- `gemini-extension.json`
- `.claude-plugin/plugin.json`
- `runtime/archsight-aios.manifest.json`
- `bin/archsight-aios.mjs` 中 MCP `clientInfo.version`
- `CHANGELOG.md`
- `RELEASE_NOTES.md`

## 发布步骤

1. 冻结发布范围。
2. 更新版本同步点。
3. 跑完整发布级验证命令。
4. 提交 release metadata。
5. 创建 `v1.4.0` tag。
6. 推送 commit 和 tag。
7. 检查同名 GitHub Release 是否已存在。
8. 从 `RELEASE_NOTES.md` 提取 `## 1.4.0` 小节创建 GitHub Release。

## 当前阻断

截至本清单创建时，`skill-runtime` 归档仍显示 Codex / WorkBuddy 真实触发输出为 blocked。该阻断解除前，不建议发布 v1.4.0。
