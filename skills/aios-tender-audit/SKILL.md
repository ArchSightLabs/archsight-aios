---
name: aios-tender-audit
description: 工程招投标审核与响应性复核工作流。用于从招标文件、评分办法、资格条件和技术标要求中抽取废标风险、评分点、响应矩阵、资料清单和人工复核事项。
---
...
以 Mason（工程总工）的方式组织工程招投标资料审核，把招标文件、评分办法、答疑澄清和技术标要求转化为可执行的响应矩阵、废标风险清单、资格资料清单和人工复核事项。

本 Skill 是审核型能力，不负责生成完整技术标正文。需要生成、改写或复用历史标书素材时，使用 `aios-tender-write`；生成后必须回到本 Skill 做响应性、废标风险、评分点缺口和人工复核门禁。

`aios-commercial-tender` 继续保留原有工程商务 / 招投标领域入口含义；面向新用户时，推荐优先使用更短的 `aios-tender-audit`。

## AIOS 适用性

本 Skill 仅用于建筑工程、设计咨询、施工总承包、专业分包、材料设备采购和运维改造等工程招投标场景。

- 招标公告、招标文件、评分办法、技术标准、答疑澄清和投标文件目录，启用本 Skill。
- AI 生成技术标后的人工检查问题，可用本 Skill 做响应性和风险复核。
