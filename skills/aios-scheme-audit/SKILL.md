---
name: aios-scheme-audit
description: 专项施工方案审核与证据链复核工作流。用于从施工方案中抽取适用范围、工序、危险源、交底要点、规范核验点、计算书缺口和专家/总工复核事项。
---

# AIOS Scheme Audit

## 目标

以 Vitruvius（建筑数字化专家）的方式整理专项施工方案，把方案文本拆成可复核的工程对象、工序控制点、危险源、交底要点和人工复核清单。

本 Skill 是审核型能力，不负责生成完整施工方案正文。需要生成、改写或复用历史方案素材时，使用 `aios-scheme-write`；生成后必须回到本 Skill 做危险源、规范 / 计算书、专家意见和交底要点门禁。

`aios-construction-scheme` 继续保留原有工程施工 / 专项方案领域入口含义；面向新用户时，推荐优先使用更短的 `aios-scheme-audit`。

本 Skill 不替代总工审查、专家论证、危大工程审批、结构计算、施工许可或安全验收。涉及结构力学、承载、稳定、变形或安全系数的数值结论必须交给 Euclid 和确定性求解器或项目计算书。

## AIOS 适用性

本 Skill 仅用于建筑工程专项施工方案、技术措施、质量安全方案和班组交底资料。

- 深基坑、模板支撑、脚手架、起重吊装、钢结构、幕墙、机电安装、混凝土浇筑、防水等施工方案，启用本 Skill。
- 普通通用流程文档、非建筑作业指导书或行政制度，不使用本 Skill。
- 如果任务要求最终合规结论、危大工程审查结论或结构安全结论，必须升级人工专家和确定性工具。

## 输入

优先收集：

- 专项施工方案、工程概况、图纸说明、计算书、专家论证意见和审批表。
- 项目条件：地区、工程类型、危险性较大分部分项工程属性、施工阶段、场地和环境约束。
- 资料定位：文件名、版本、日期、页码、章节、图号、表号或段落。
- 用户关注点：危险源、关键工序、交底、规范核验、计算书缺口、专家复核项。
- 可用 Knowledge Pack 或 `knowledge.norm_lookup` 查询输入；如存在，应优先使用其引用、版本、适用性和仲裁状态，不得只靠模型常识判断规范口径。

## 基础版模式

用于工程业务资料处理、项目复核辅助、部门工作流整理或模板沉淀时，按本节收口。可复制的基础提示词见 `prompts/basic-prompt.md`。

最小可用输入：

- 脱敏后的专项施工方案片段、工程概况、施工工艺、安全措施、专家修改说明、AI 方案编制试用反馈或计算书目录。
- 文件名、版本、日期、章节 / 页码 / 图号 / 表号。
- 用户要复核的问题，例如危险源、交底要点、专家意见回查、计算书缺口、AI 生成失准原因。
- 人工复核岗位，例如技术负责人、总工、安全负责人、质量负责人、专家或资料员。

基础输出优先级：

1. 全程使用“AI 辅助施工方案复核”口径，不使用“AI 审查施工方案”。
2. 如资料是 AI 方案编制试用反馈，先输出 AI 生成失准原因复盘，再给后续资料包和回查清单建议。
3. 如资料包含专家论证修改说明，先拆成回查点，再回查正文、附图、计算书和验收要求是否有对应依据。
4. 不输出“合格 / 不合格 / 已通过审查”，不确认计算正确，不替代总工、专家或审批主体。

## 标准详版报告与输出自检

短指令触发本 Skill 时，默认输出标准详版报告，不要压缩成摘要。标准详版报告必须包含：资料来源清单、主分析表 / 清单 / 台账、资料缺口、人工复核岗位、AI 不应下结论事项、输出自检。

最小详度要求：

- 资料足够时，关键工序与控制点、危险源与控制措施或规范 / 计算书 / 专家复核清单至少展开 3 条可定位事项；资料不足时，列出全部可定位事项并说明不足原因。
- 主分析表必须保留“事项 / 类型 / 方案原文 / 证据 / 工具结果 / 处理建议 / 建议复核岗位”等字段，不得只写概括段落。
- 输出自检必须逐项确认是否包含资料来源、主分析表 / 清单 / 台账、资料缺口、人工复核岗位、AI 不应下结论事项；缺一项时先补齐再结束。

## Capability 与证据

- 每个危险源、控制措施和交底要点必须绑定方案原文证据。
- 涉及规范条文、地方规程、强制性要求或危大工程判断时，调用或要求 `knowledge.norm_lookup`；如果有已编译 Knowledge Pack，应记录 `knowledgePackPath`、命中条文、`sourceVersion` 和 `applicability`；没有工具结果时标注 `需核验`。
- 涉及力学计算、承载力、稳定、变形、荷载或边界条件时，不得口算；输出 `处理建议：转 Euclid 结构复核`，并要求 `aios-structural` 或项目计算书证据。
- Agent 冲突或证据缺失按中文化的 `判断事项 / 证据 / 工具结果 / 处理建议` 输出。

## 工作流

1. 建立资料来源清单：列出方案版本、附件、计算书、审批 / 论证文件和定位方式。
2. 提取工程对象和适用范围：施工部位、专业、阶段、限制条件和前置条件。
3. 拆解关键工序：按施工顺序提取质量控制点、安全控制点、验收点和交底要点。
4. 建立危险源清单：危险源、触发条件、方案已写措施、缺失证据和人工复核岗位。
5. 标注规范核验点：只列需核验问题，不在缺少工具结果时给出合规结论。
6. 如存在 Knowledge Pack，使用 `knowledge.norm_lookup` 对规范 / 审查口径问题做查询，并把 `found`、`not_found`、`conflict`、`inapplicable` 或 `need_context` 写入证据仲裁。
7. 标注计算书和专家复核缺口：力学参数、荷载组合、支撑体系、设备能力、地质条件等。
8. 输出班组交底摘要：把已确认的操作红线转为可执行提醒，保留来源证据。

## 输出格式

默认输出：

1. 结论
2. 资料来源清单
3. 工程概况与适用范围
4. 关键工序与控制点
5. 危险源与控制措施
6. 规范 / 计算书 / 专家复核清单
7. 班组交底要点
8. 证据仲裁

复核条目格式：

```text
事项：
类型：规范核验 / 计算书 / 专家论证 / 现场条件 / 资料缺口
方案原文：
证据：
工具结果：
处理建议：
建议复核岗位：
```

## 约束

- 不输出“方案合格 / 不合格 / 已通过审查”等最终结论。
- 不替代总工、专家、监理、安监或审批主体。
- 不使用 LLM 口算结构安全或设备能力结论。
- 不在缺少规范版本、项目条件或工具证据时给出合规判断。
- 不把方案未写内容补猜成现场事实。
