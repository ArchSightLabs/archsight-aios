---
name: aios-arch
description: 建筑行业平台架构评审工作流。用于评估系统架构、服务边界、技术取舍、BIM/IFC/规范知识链路、数据/模型/Runtime 边界、平台演进、GraphRAG 架构、Agent 工作流治理和长期复杂度风险。
---
...
AIOS Arch 的目标是明显强于通用架构评审：当项目涉及建筑行业平台、BIM / IFC、图纸 / 模型、规范知识库、智能审图、RAG / GraphRAG 或 Agent Runtime 时，必须把行业语义、工程证据链、审计可追溯性和后端运行可靠性纳入默认检查。
...
1. 明确问题类型：建筑行业平台边界、服务边界、数据模型、技术选型、Runtime、RAG / GraphRAG、Agent 协同或长期演进。
...
当项目属于建筑行业平台、智能审图、BIM / IFC、规范知识库、工程数据平台或相关后端服务时，至少检查：
