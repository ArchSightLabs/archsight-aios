...
- 使用本 Skill 执行扫描、归一化、基线差分、临时预算和门禁。
- 使用 `aios-arch` 解释热点是深 Module、合理复杂度还是职责混杂。
...
4. 生成 `architecture-health.json`、Markdown、SARIF、依赖图和循环依赖清单。
5. 只让 `measured` 的确定性违规触发 fail；`inferred` 只进入建议；缺少里程碑必需证据时标记 `unverified` 并 HOLD。
6. 对照基线区分 `new / worsened / improved / retained / resolved / budgeted`。
7. 校验临时预算的作用域、负责人、原因、上限和到期时间；过期预算不得放行。
8. 把需要业务解释的热点交给 `aios-arch`，同时保留原始事实和证据定位。
...
- `hold`：基线不兼容、里程碑必需证据缺失、必需扫描器失败或输入不可验证。
